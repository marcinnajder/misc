...
import { readString, IO, writeString, main, ioMonadOps } from "./IOMonad";
...
function ioWithBind() {
  function io() {
    const strIO: IO<string> = readString();

    const voidIO: IO<void> = strIO.bind(s => {
      const upper = s.toUpperCase();
      return writeString(upper);
    });
...
function ioWithGenerator() {
  function* generator(): IterableIterator<IO<any>> {
    const s: string = yield readString();
    const upper = s.toUpperCase();
    return writeString(upper);
  }

  const ioMonad = do_<IO<void>>(generator, ioMonadOps);
  main(ioMonad);
}
