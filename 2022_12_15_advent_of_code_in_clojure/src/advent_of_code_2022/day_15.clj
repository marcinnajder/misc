...
(defn find-free-space-on-board [board [column row]]
  (let [center-result (find-free-space-in-column (get board column '()) row)]
    (condp = (:type center-result)
      :not-found {:type :not-found :column column}
      :occupied {:type :occupied}
      :found (find-free-space-on-left-and-right board [column (:row center-result)]))))


(defn drop-sand [board floor-row]
  (loop [board board
         index 0]
    (let [result (find-free-space-on-board board [500 0])]
      (condp = (:type result)
        :occupied index
        :found (recur (insert-straight-line board [(:pos result) (:pos result)]) (inc index))
        :not-found
        (if (nil? floor-row)
          index
          (recur (insert-straight-line board [[(:column result) (dec floor-row)] [(:column result) (dec floor-row)]]) (inc index)))))))
