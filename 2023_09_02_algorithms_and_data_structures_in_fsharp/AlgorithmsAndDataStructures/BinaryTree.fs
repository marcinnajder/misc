module AlgorithmsAndDataStructures.BinaryTree
...
let tree = Node(0, Node(1, Node(3, Tip, Tip), Node(4, Tip, Tip)), Node(2, Tip, Tip))

[ preorder; inorder; postorder ] |> Seq.map (fun f -> f tree |> Seq.toArray) |> Seq.toArray
=== [| [| 0; 1; 3; 4; 2 |]; [| 3; 1; 4; 0; 2 |]; [| 3; 4; 1; 2; 0 |] |]



let rec levelorder (nodes: seq<_>) =
    seq {
        let children = ResizeArray<_>()
        for node in nodes do
            ...
            | Node (value, left, right) ->
                ...
                children.Add(left)
                children.Add(right)
        if children.Count > 0 then yield! levelorder children
    }

[ tree ] |> levelorder |> Seq.toArray === [| 0; 1; 2; 3; 4 |]


let rec isValid tree =
    match tree with
    | Tip
    | Node (_, Tip, Tip) -> true
    | Node (value, left, right) ->
        match left, right with
        | Node (leftValue, _, _), Tip -> value > leftValue && isValid left
        | Tip, Node (rightValue, _, _) -> value < rightValue && isValid right
        | Node (leftValue, _, _), Node (rightValue, _, _) ->
            value > leftValue && value < rightValue && isValid left && isValid right
        | _ -> false

"[2,1,3]" |> parseTree |> isValid === true
"[5,1,4,null,null,3,6]" |> parseTree |> isValid === false
