import { Monad, MonadOperations } from "./monad";
import { some, Option } from "./option";
import { optionMonadOps, resultMonadOps } from "./monadImplementations";
import { ResultS } from "./result";

export const forM = mapM;

export function replicateM<T>(count: number, m: Monad<T>, ops: MonadOperations): Monad<T[]> {
...
export function when(condition: boolean, m: Monad<void>, ops: MonadOperations): Monad<void> {
  return condition ? m : ops.return_(undefined);
}

export function liftM<T, T2>(f: (a: T) => T2, ops: MonadOperations): (a: Monad<T>) => Monad<T2> {
  return m => ops.bind(m, v => ops.return_(f(v)));
}

export function liftM2<T1, T2, T3>(f: (a: T1, b: T2) => T3, ops: MonadOperations): (a: Monad<T1>, b: Monad<T2>) => Monad<T3> {
...
export function reduceM<T, TA>(items: T[], f: (prev: TA, item: T) => Monad<TA>, seed: TA, ops: MonadOperations): Monad<TA> {
  const next: (index: number, acc: TA) => Monad<TA> = (index, acc) => {
    if (index === items.length) {
